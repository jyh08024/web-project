				...
				<form action="/multi" method="post">
					...
					<ul class="info multiple">
						...
						<?php foreach ($data as $key => $value): ?>
							<input type="hidden" name="idxs[]" value="<?php echo $value['idx'] ?>">
							<li>
								<p>
									<label>품목명: </label>
									<span><?php echo $value['info']['name'] ?></span>
								</p>
								<p>
									<label>대여/반납일(시간): </label>
									<span><?php echo $value['start'] ?> ~ <?php echo $value['end'] ?></span>
								</p>
								<p>
									<label>대여/반납지점: </label>
									<span><?php echo $value['start_place'] ?>/<?php echo $value['end_place'] ?></span>
								</p>
								<p>
									<label>품목금액: </label>
									<span><?php echo number_format($value['pay']['ori']) ?></span> 원
								</p>
								<p>
									<label>할인/할증금액: </label>
									<span><?php echo number_format(-$value['pay']['sale']) ?></span> 원
								</p>
								<p>
									<label>결제금액: </label>
									<span>= <?php echo number_format($value['pay']['salePay']) ?></span> 원
								</p>
								<p>
									<label>회원 할인금액: </label>
									<span><?php echo number_format(-$value['pay']['member']) ?></span> 원
								</p>
								<p>
									<label>총 결제금액: </label>
									<span>= <?php echo number_format($value['pay']['realPay']) ?></span> 원
								</p>
							</li>
							<?php $total += $value['pay']['realPay'] ?>
						<?php endforeach ?>
						<li>
							<p>
								<label>총 다중결제금액:</label>
