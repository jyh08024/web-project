	<div id="content">
		<div class="w100">
			<div class="card">			
				...
				<form action="/post/cardPay" method="post">
					<ul class="info">
						<li>
							<label>품목명 :</label>
							<span><?= $pro['name'] ?></span>
						...
						<li>
							<label>렌탈/반납지점</label>
							<span><?= $res['begin'] ?>/<?= $res['arrive'] ?></span>
						</li>
						<li>
							<label>품목금액 :</label>
							<span><?= number_format($pro['price']) ?></span> 원
						</li>
						<li>
							<label>할인/할증금액 :</label>
							<span><?= number_format(-$res['priceInfo']['sale']) ?></span> 원
						</li>
						<li>
							<label>결제금액 :</label>
							<span>= <?= number_format($res['priceInfo']['salePrice']) ?></span> 원
						</li>
						<li>
							<label>회원 할인금액 :</label>
							<span>- <?= number_format($res['priceInfo']['userSale']) ?></span> 원
						</li>
						<li>
							<label>총 결제금액 :</label>
							<h2>= <?= number_format($res['priceInfo']['realPay']) ?></h2> 원
						</li>
					</ul>

					<input type="hidden" name="pay_type" value="card" class="pay_type">

					<div class="pay">
						<ul class="row">
							<li class="col-md-6 active" onclick="$(`.pay_type`).val('card')"><a href="#cardpay" data-toggle="tab" class="col-md-12 blueBtn">카드결제</a></li>
							<li class="col-md-6"><a href="#cashpay" data-toggle="tab" class="check col-md-12" onclick="$(`.pay_type`).val('bank')">무통장입금</a></li>
						</ul>
